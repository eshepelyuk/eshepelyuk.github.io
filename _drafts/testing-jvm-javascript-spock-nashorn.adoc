...
So, driven by the need for comfort JavaScript testing tools we've created this eetup I am going to describe below.

== Customized JavaScript executor

There's no need to conform standards in non browser JS environments, so usually
developers extend engine with custom functions, built-in variables etc.
